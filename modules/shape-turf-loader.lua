-- Turf Mod Loader by Parigo-57
-- Injects randomly a turf mod located in "content\\shape-turf-loader" if the game started.

-- path, which should contain the different turf mods (extracted files, no CPKs!)
local contentPath                     = ".\\content\\shape-turf-loader"
local TURF_MODS_CSV                   = "shape-mods.csv"
...
local NEXT_SB_KEY                     = 0x21 -- Page Up
...
function isTurfFile(filename)
    if  string.match(filename, "Asset\\model\\bg\\st%d%d%d\\sourceimages\\tga\\#windx11\\")
    then
        return true
    else
        return false
...
Version %s
          ...
          [PageUp]  to manually select next shape turf mod
             [DEL]  to clear info messages
               [3]  to reload map %s files
               [4]  (not use this num)map %s files 
               [9]  to enable/disable automatically choosing a random turf mod per match

      Selected Shape Turf Mod: %s
      Enabled Random Shape Turf Mod: %s (Load "No Mod": %s)

%s
]],
        version, TURF_MODS_CSV, TEAM_TURF_MODS_CSV, selectedTurf, selectedLight, isTeamTurfModEnabled,
        isTurfModRandomEnabled, loadNoTurf, isLightModRandomEnabled, loadNoLight, infoText)
end
